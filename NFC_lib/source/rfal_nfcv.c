...
#define QX3900_NFCV_BLOCK_LEN 4

rfalNfcvListenDevice *QXnfcvDev;
void Qx3900NFC_UID_SAVE(rfalNfcvListenDevice *nfcvDev){
	QXnfcvDev = nfcvDev;
}

void Qx3900NFC_AllBlock_Write(rfalNfcvListenDevice *nfcvDev){
	uint8_t reqFlag;
    ReturnCode            err;
    uint16_t              rcvLen;
    uint8_t               blockNum = 1;
    uint8_t               rxBuf[ 1 + QX3900_NFCV_BLOCK_LEN + RFAL_CRC_LEN ];                        /* Flags + Block Data + CRC */
    uint8_t               *uid;
    uint8_t               wrData[QX3900_NFCV_BLOCK_LEN] = { 0x00, 0x01, 0x02, 0x03 };             /* Write block example */

    if(QXnfcvDev == NULL) return;

    uid     = nfcvDev->InvRes.UID;
    reqFlag = RFAL_NFCV_REQ_FLAG_DEFAULT;
    for(blockNum = 0;blockNum < 32;blockNum++){
#ifdef QX_3900_TEST
    	myprintf(5,"         [Block NUM : 0X%x]\r\n",blockNum );
    	if(wrData[0] < 0xff)  	wrData[0]++;
    	else					wrData[0] = 0x00;
    	if(wrData[1] < 0xff)  	wrData[1]++;
    	else					wrData[1] = 0x00;
    	if(wrData[2] < 0xff)  	wrData[2]++;
    	else					wrData[2] = 0x00;
    	if(wrData[3] < 0xff)  	wrData[3]++;
    	else					wrData[3] = 0x00;

    	err = rfalNfcvPollerReadSingleBlock(reqFlag, uid, blockNum, rxBuf, sizeof(rxBuf), &rcvLen);
    	myprintf(5," Read Block: %s %s\r\n", (err != RFAL_ERR_NONE) ? "FAIL": "OK Data:", (err != RFAL_ERR_NONE) ? "" : hex2Str( &rxBuf[1], QX3900_NFCV_BLOCK_LEN));

    	err = rfalNfcvPollerWriteSingleBlock(reqFlag, uid, blockNum, wrData, sizeof(wrData));
    	myprintf(5," Write Block: %s Data: %s\r\n", (err != RFAL_ERR_NONE) ? "FAIL": "OK", hex2Str( wrData, QX3900_NFCV_BLOCK_LEN) );

...
static void st25r3916comRx( uint8_t* rxBuf, uint16_t rxLen )
{
    ...
    if( rxLen > 0U )
    {
    ...
    #ifdef ST25R_COM_SINGLETXRX
        RFAL_MEMSET( &comBuf[comBufIt], ST25R3916_MOSI_IDLE, RFAL_MIN( rxLen, (uint16_t)(ST25R3916_BUF_LEN - comBufIt) ) ); /* Clear outgoing buffer                       */
        ...
        {
            RFAL_MEMSET( rxBuf, ST25R3916_MOSI_IDLE, rxLen );                                                    /* Clear outgoing|incoming buffer                         */
